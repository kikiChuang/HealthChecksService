...
using HealthCheckHostServer.Extensions;
...
using HealthCheckConfiguration = HealthCheckHostServer.Core.Data.HealthCheckConfiguration;
using HealthCheckExecution = HealthCheckHostServer.Core.Data.HealthCheckExecution;
using HealthCheckExecutionHistory = HealthCheckHostServer.Core.Data.HealthCheckExecutionHistory;
using HealthChecksDb = HealthCheckHostServer.Core.Data.HealthChecksDb;
...
namespace HealthCheckHostServer.Core.HostedService
{
    internal class HealthCheckReportCollector : IHealthCheckReportCollector
    {
        ...
        public async Task Collect(int callTimes, CancellationToken cancellationToken)
        {
            using (_logger.BeginScope("HealthReportCollector is collecting health checks results."))
            {
                var healthChecks = await (from con in _db.Configurations
                                          join hcparam in _db.HealthCheckParameters on con.Id equals hcparam.ConfigID
                                          where hcparam.ExecutionTime != 0 && callTimes % hcparam.ExecutionTime == 0 
                                          select con).OrderBy(h => h.Id).ToListAsync();
                ...
                foreach (var item in healthChecks)
                {
                    ...
                    {
                        await _healthCheckFailureNotifier.NotifyDown(item.Id, item.Name, healthReport);
                    }
                    else
                    {
                        if (await HasLivenessRecoveredFromFailure(item))
                        {
                            await _healthCheckFailureNotifier.NotifyWakeUp(item.Id, item.Name);
                        }
                    }
